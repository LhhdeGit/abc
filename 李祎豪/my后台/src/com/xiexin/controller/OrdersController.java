...
public Map addOrders(HttpServletRequest request, Orders orders){
   ...
   //调用service做添加
    ...
    if(i==1){
        codeMap.put("code",0);
        codeMap.put("msg","您的提交已经收到，请耐心等待");
        return codeMap;
        //作业3：当顾客比较着急，连续提交，这个学名叫做ajax重复提交
        //第一次提交完毕，将提交的按钮变为不可点击状态，并且提交2次变为已提交订单，请稍等（前端处理）
        //第二次提交的时候（防黑客，点击速度比较快）,给他返回亲，我知道您很急，订单已经接收了，请稍等即可 (后端处理)
    //思路：先查询该手机号有没有订单状态，保存到redis中，有了就返回给前端，没有再执行新增
    }else{
        codeMap.put("code",400);
        codeMap.put("msg","由于网络故障，未能添加成功，请您重新来一次");
        return codeMap;
    }
